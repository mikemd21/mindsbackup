jQuery(document).ready(function() {
    ...
    $('#rfform').on('submit', function(event){
		event.preventDefault();
		...
		$.ajax({
			...
			success:function(data){
                if (data == "OK"){
                    $('#rfform')[0].reset();
                    $('#alert_action2').fadeIn().html('<div class="alert alert-success alert-dismissable"><b>Correcto: </b>Se creó el usuario correctamente</div>');
                    $(".alert").fadeTo(2000, 500).slideUp(500, function(){
                        $(this).remove();
                    });
                }else{
                    $('#alert_action2').fadeIn().html('<div class="alert alert-danger alert-dismissable"><b>Error: </b>'+data+'</div>');
                    $(".alert").fadeTo(2000, 500).slideUp(500, function(){
                        $(this).remove();
                    });
                }
